...
func TestTaskRunSigner_SignTaskRun(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := ts.SignTaskRun(ctx, tr, nil, nil); (err != nil) != tt.wantErr {
...
func TestTaskRunSigner_Transparency(t *testing.T) {
	for _, format := range []string{"in-toto", "tekton"} {
		...
		if err := ts.SignTaskRun(ctx, tr, nil, nil); err != nil {
			t.Errorf("TaskRunSigner.SignTaskRun() error = %v", err)
		}
		...
		if err := ts.SignTaskRun(ctx, tr2, nil, nil); err != nil {
			t.Errorf("TaskRunSigner.SignTaskRun() error = %v", err)
		}
		...
		if err := ts.SignTaskRun(ctx, tr3, nil, nil); err != nil {
			t.Errorf("TaskRunSigner.SignTaskRun() error = %v", err)
		}
